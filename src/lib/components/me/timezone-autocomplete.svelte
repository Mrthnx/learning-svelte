<script lang="ts">
	import { GMT_VALUES } from '$lib/shared/constants';
	import { createEventDispatcher } from 'svelte';
	import { ChevronDown, X, Clock } from 'lucide-svelte';
	...
	const dispatch = createEventDispatcher<{
		change: { key: string; value: string; region: string };
	}>();
	...
	let selectedTimezone = $derived(GMT_VALUES.find((tz) => tz.key === value));

	function selectTimezone(timezone: (typeof GMT_VALUES)[0]) {
		value = timezone.key;
		isOpen = false;
		searchTerm = '';
		dispatch('change', { key: timezone.key, value: timezone.value, region: timezone.region });
	}

	function clear() {
		value = null;
		searchTerm = '';
		dispatch('change', { key: '', value: '', region: '' });
	}
		...
		>
			{#if selectedTimezone}
				<span class="flex items-center gap-2">
					...
					<span class="font-medium">{selectedTimezone.value}</span>
					<span class="text-muted-foreground">- {selectedTimezone.region}</span>
				</span>
			{:else}
				<span class="text-muted-foreground">{placeholder}</span>
			{/if}
			<div class="flex items-center gap-1">
				{#if selectedTimezone && !disabled}
					<button
						type="button"
						onclick={(e) => {
							e.stopPropagation();
		...
		{#if isOpen}
			...
			>
				...
				{#if filteredTimezones.length === 0}
					<div class="py-6 text-center text-sm text-muted-foreground">No timezones found.</div>
				{:else}
