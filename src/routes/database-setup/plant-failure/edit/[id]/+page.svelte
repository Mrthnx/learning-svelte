<script lang="ts">
	...
	let plantFailure = $state<PlantFailure | null>(null);
	...
	onMount(async () => {
		try {
			plantFailure = await plantFailureService.getById(Number(id));
		} catch (error: any) {
			console.error('Error loading plant failure:', error);
			toast.error(error.message || 'Failed to load plant failure');
			notFound = true;
		} finally {
			isLoading = false;
		}
	...
	async function handleSubmit(data: PlantFailure) {
		await plantFailureService.update(Number(id), data);
	...
	async function confirmDelete() {
		isDeleting = true;
		try {
			await plantFailureService.delete(Number(id));
...
<div class="container mx-auto max-w-4xl p-6">
	<div class="mb-6">
		...
		<div class="flex items-start justify-between">
			<div>
				<h1 class="text-3xl font-bold">Edit Plant Failure</h1>
		...
		<Card.Card>
			<Card.CardContent class="flex items-center justify-center py-12">
				<div class="flex flex-col items-center gap-4">
					...
					></div>
					<p class="text-sm text-muted-foreground">Loading plant failure...</p>
				</div>
			</Card.CardContent>
		</Card.Card>
	{:else if notFound}
		<Card.Card>
			<Card.CardContent class="flex items-center justify-center py-12">
				<div class="flex flex-col items-center gap-4 text-center">
					...
					<div>
						<h3 class="mb-2 text-lg font-semibold">Plant Failure Not Found</h3>
						<p class="text-sm text-muted-foreground">
							The plant failure association you're looking for doesn't exist or has been deleted.
						</p>
					</div>
					<Button onclick={() => goto('/database-setup/plant-failure')} class="gap-2">
						<ArrowLeft class="h-4 w-4" />
						Back to Plant Failures
					</Button>
				</div>
...
<AlertModal
	...
	description="Are you sure you want to delete this association? This action cannot be undone."
	buttons={[
...
<AlertModal
	...
	description="The plant failure association has been updated successfully. What would you like to do next?"
	buttons={[
